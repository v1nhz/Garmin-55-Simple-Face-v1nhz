...
class LunarConverter {
    ...
    // Năm gốc dữ liệu (giống LunarConver.c: phần tử đầu mảng)
    var lunar_month_days as Lang.Array<Lang.Number>;
    var solar_1_1 as Lang.Array<Lang.Number>;

    function initialize() {
        // Dữ liệu từ LunarConver.c - phần tử đầu = 1887 (năm gốc để tính index)
        lunar_month_days = [
    ...
    // Lấy bits (length bit từ vị trí shift) - giống GetBitInt trong C
    function GetBitInt(data, length, shift) {
        return (data & (((1 << length) - 1) << shift)) >> shift;
    }

    // Chuyển ngày dương lịch sang số ngày (Julian day number style) - giống SolarToInt trong C
    ...
    // Chuyển số ngày sang ngày dương lịch - giống SolarFromInt trong C
    ...
    // Âm lịch -> Dương lịch (logic từ LunarToSolar trong C)
    function LunarToSolar(lunar) {
        var baseYear = lunar_month_days[0];
        var index = lunar.lunarYear - baseYear;
        if (index < 0 || index >= lunar_month_days.size()) {
            return null;
        }
    ...
    // Dương lịch -> Âm lịch (logic từ SolarToLunar trong C)
    function SolarToLunar(solar) {
        var baseYear = solar_1_1[0];
        var index = solar.solarYear - baseYear;
        if (index < 0 || index >= solar_1_1.size()) {
            return null;
        }
